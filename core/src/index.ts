import { CodeAnalysisResult, AnalyzeCodeFunction, FormatCodeFunction } from './types/index.js';

/**
 * Analyze JavaScript/TypeScript code
 */
export const analyzeCode: AnalyzeCodeFunction = (code: string, fileName: string): CodeAnalysisResult => {
  ...
  const functionRegex = /(?:function\s+(\w+)|(\w+)\s*:\s*(?:async\s+)?(?:\([^)]*\)\s*=>|\([^)]*\)\s*{)|(?:async\s+)?(\w+)\s*\([^)]*\)\s*{|(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?\([^)]*\)\s*=>)/g;
  ...
  while ((match = variableRegex.exec(code)) !== null) {
    ...
    if (varName && !functions.includes(varName) && !variables.includes(varName)) {
